	...
	public function kwpsbb_add_button()
	{
		...
		//	if(!empty($_GET['product-page']) ){
			...
			//	$_SESSION['product_page'] = intval($_GET['product-page']);
				$_SESSION['product_page121'] = intval($_GET['paged']);
			}
			else
				$_SESSION['product_page121'] = 1;
				
				//var_dump($_SESSION['product_page121']);
				
			}
			else
			{
				//var_dump("hello team leader koonkhony!!!");
				...
				//var_dump($urldata['query']);
				//	if(!empty($urldata['query']) && preg_match('/product-page/', $urldata['query'])){
				if(!empty($urldata['path']) && preg_match('/page/', $urldata['path'])){
					//var_dump("team leader koon dady?");
				...
				$val = substr($val, 0, $si);
						...
						$fib = strpos($urldata['path'], 'page');
						$valb = substr($urldata['path'], $fib+5);
						if(strpos($valb, '/')){
				$ffib = strpos($valb, '/');
				
				$valb = substr($valb, 0, $ffib);
							var_dump($valb);
				}
					else
							$valb = substr($valb, 0);
				...
				$_SESSION['product_page'] = intval($valb);
			}
			else
	...
	//	var_dump($_SESSION['product_page121']);
		...
		if(preg_match('/product=/', $_SERVER['REQUEST_URI']))
		{
			...
			jQuery(document).ready(function(){
				jQuery('.woocommerce-product-gallery__wrapper').append('<a href=\"".esc_url(get_home_url()."/?product_cat=".$_SESSION['product_cat']."&paged=".$_SESSION['product_page121'])."\" class=\"button sbb\">Back to Store  </a>');
			});
			";
		 ...
		 if(preg_match('/product/', $_SERVER['REQUEST_URI']))
		{
				$addscript = "
			jQuery(document).ready(function(){
				jQuery('.woocommerce-product-gallery__wrapper').append('<a href=\"".esc_url(get_home_url()."/product-category/".$_SESSION['product_cat']."/page/".$_SESSION['product_page'])."\" class=\"button sbb\">Back to Store  </a>');
			});
			";
		}
		 else
